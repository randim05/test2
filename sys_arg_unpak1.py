...
while main_loop_flag:
    ...
    if pay_type == "diff" and principal and periods and interest and not payment:
        ...
        while now_month <= periods:
            dif_pay = principal / periods + interest * (principal - (principal * (now_month - 1)) / periods)
            print(f'Month {now_month}: payment is {dif_pay}')
            now_month += 1
            sum_pay += dif_pay
        print()
        print(f'Overpayment = {sum_pay - principal}')
    if pay_type == "annuity":
        if payment and periods and interest and not principal:
            ...
            print(f'Overpayment = {int(principal) - payment * periods}')
            ...
            print(f'Overpayment = {principal - payment * periods}')
